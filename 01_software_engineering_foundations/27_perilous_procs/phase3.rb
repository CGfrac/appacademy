...
# The hash contains procs as both keys and values. 
# The method should return a new sentence where each word of the input sentence is changed 
# by a value proc if the original word returns true when passed into the key proc. 
...
def picky_procipher(sentence, hash)
    ...
    sentence.split.each do |word|
        ...
        hash.each_key do |prc|
            if prc.call(word)
                new_word = hash[prc].call(new_word)
                break
            end
        end
        new_sentence << new_word
